...
function SectionDivider({ title }: { title: string }) {
  return (
    <div className="flex items-center gap-3 sm:gap-4 py-2">
      <div className="flex-1 h-px bg-gradient-to-r from-transparent via-[#4583DA]/40 to-transparent" />
      <span className="text-[#4583DA] text-[10px] sm:text-xs font-bold tracking-[0.2em] uppercase whitespace-nowrap flex items-center gap-2">
        <span className="w-1.5 h-1.5 rounded-full bg-[#4583DA]" />
        {title}
        <span className="w-1.5 h-1.5 rounded-full bg-[#4583DA]" />
      </span>
      <div className="flex-1 h-px bg-gradient-to-r from-transparent via-[#4583DA]/40 to-transparent" />
    ...
    >
      ...
      <div className="form-wrapper w-full max-w-4xl">
        ...
        >
          ...
          {/* ── HEADER ── */}
          <div
            className="px-6 sm:px-10 py-5 sm:py-6 text-center relative overflow-hidden"
            style={{ background: "linear-gradient(135deg, #1a3f7a 0%, #2d5cad 60%, #4583DA 100%)" }}
          >
            <h1 className="form-title text-2xl sm:text-3xl font-semibold text-white tracking-wide mb-1">
              Student Enrollment
            </h1>
          ...
          {/* ── FORM BODY ── */}
          <div className="px-4 sm:px-8 md:px-10 py-6 sm:py-8 space-y-5 sm:space-y-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mb-3 sm:mb-4">
                <div>
                  ...
                  <input type="text" name="firstName" value={formData.firstName} onChange={handleChange} className={inputBase} />
                </div>
                <div>
                  <label className={labelBase}>Last Name</label>
                  <input type="text" name="lastName" value={formData.lastName} onChange={handleChange} className={inputBase} />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
                ...
                <div>
                  ...
                  <div className="flex gap-1.5 sm:gap-2 mt-1.5">
                    <select name="month" value={formData.month} onChange={handleChange} className={selectBase}>
                      <option value="">MM</option>
                      {Array.from({ length: 12 }, (_, i) => (
                        <option key={i} value={i + 1}>{i + 1}</option>
                      ))}
                    </select>
                    <select name="day" value={formData.day} onChange={handleChange} className={selectBase}>
                ...
                <div>
                  ...
                  <select name="gender" value={formData.gender} onChange={handleChange} className={selectBase + " mt-1.5"}>
                ...
                <div>
                  ...
                  <select name="grade" value={formData.grade} onChange={handleChange} className={selectBase + " mt-1.5"}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              <div>
                ...
                <input name="guardianName" value={formData.guardianName} onChange={handleChange} className={inputBase} />
              </div>
              <div>
                <label className={labelBase}>Email</label>
                <input name="email" value={formData.email} onChange={handleChange} className={inputBase} />
              </div>
              <div>
                <label className={labelBase}>Phone</label>
                <input name="phone" value={formData.phone} onChange={handleChange} className={inputBase} />
              </div>
              <div>
                <label className={labelBase}>Address</label>
                <input name="address" value={formData.address} onChange={handleChange} className={inputBase} />
              </div>
            </div>
            ...
            <div>
              ...
              <textarea name="comments" value={formData.comments} onChange={handleChange} className={inputBase + " h-24 resize-none"} />
            </div>

            {/* Submit */}
            <div className="flex justify-center sm:justify-end pt-2">
              <button className="w-full sm:w-auto px-8 py-3 bg-[#2E3363] rounded-xl text-white text-sm font-semibold tracking-widest uppercase hover:shadow-lg transition">
                SUBMIT
              </button>
            </div>
