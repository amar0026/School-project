...
// ── Data ─────────────────────────────────────────────────────────────────────
const tabs = ["Campus", "Events", "Sports", "Cultural", "Class Activities", "Videos"];
...
const ImageTile: React.FC<{
  ...
  return (
    ...
    >
      ...
      />
      <div className="absolute inset-0" style={{ background: hovered ? "rgba(0,0,0,0.35)" : "rgba(0,0,0,0)", transition: "background 0.35s ease" }} />
...
const SectionBlock: React.FC<{
  ...
  <div className="rounded-2xl overflow-hidden" style={{ border: "1.5px solid #dbeafe", boxShadow: "0 6px 32px rgba(30,64,175,0.07), 0 2px 8px rgba(0,0,0,0.04)" }}>

    {/* Header */}
    <div className="flex items-center gap-3 px-4 sm:px-5 py-3 sm:py-4" style={{ background: "linear-gradient(90deg, #eff6ff 0%, #ffffff 100%)", borderBottom: "1.5px solid #dbeafe" }}>
      <div className="w-1.5 h-6 rounded-full bg-[#1E40AF]" />
      <h3 className="text-base sm:text-lg font-semibold text-[#1E40AF] tracking-wide" style={{ fontFamily: "'Playfair Display', serif" }}>
        {title}
      </h3>
    </div>
    ...
    <div className="p-3 sm:p-4 bg-white">
      ...
      <div className="grid grid-cols-2 gap-2 sm:hidden">
        {images.map((src, i) => (
          <ImageTile key={i} src={src} alt={`${title} ${i + 1}`} onClick={() => onImageClick(src)} className="h-32" />
        ))}
      </div>

      {/* ── Tablet sm–md ── */}
      <div className="hidden sm:grid md:hidden grid-cols-3 gap-3" style={{ gridTemplateRows: "160px 160px" }}>
        <ImageTile src={images[0]} alt={title} onClick={() => onImageClick(images[0])} className="col-span-1 row-span-2" />
        {images.slice(1, 5).map((src, i) => (
          <ImageTile key={i} src={src} alt={`${title} ${i + 2}`} onClick={() => onImageClick(src)} />
        ))}
      </div>

      {/* ── Desktop md+ : defined row heights so tiles fill correctly ── */}
      <div className="hidden md:grid grid-cols-3 gap-3" style={{ gridTemplateRows: "220px 220px" }}>
        {/* Large feature tile — spans both rows */}
        <ImageTile src={images[0]} alt={title} onClick={() => onImageClick(images[0])} className="col-span-1 row-span-2" />
        {/* 4 equal tiles in 2×2 on the right */}
        {images.slice(1, 5).map((src, i) => (
          <ImageTile key={i} src={src} alt={`${title} ${i + 2}`} onClick={() => onImageClick(src)} />
        ))}
      </div>
...
const Gallery: React.FC = () => {
  ...
  return (
    <section
      ...
      style={{ background: "linear-gradient(160deg, #f0f6ff 0%, #f8faff 60%, #eef3ff 100%)", fontFamily: "'DM Sans', sans-serif" }}
    >
      ...
      <div className="max-w-6xl mx-auto px-3 sm:px-6">
        ...
        <div className="text-center mb-8 sm:mb-10">
          <div className="flex items-center justify-center gap-3 mb-2">
            ...
            <p className="text-[11px] sm:text-[13px] font-bold tracking-[0.3em] uppercase text-blue-400">
              School Memories
            </p>
            <div className="h-0.5 w-10 bg-[#1E40AF]" />
          </div>

          <h2 className="text-2xl sm:text-4xl font-semibold text-[#1E40AF]" style={{ fontFamily: "'Playfair Display', serif" }}>
            Our Latest Gallery
          </h2>
          <div className="flex justify-center items-center gap-2 mt-3">
            <div className="w-10 sm:w-12 h-px bg-blue-200" />
        ...
        {/* Tabs */}
        <div
          className="flex gap-1 overflow-x-auto pb-1 mb-6 bg-white rounded-2xl p-2 shadow-sm"
          style={{ border: "1.5px solid #dbeafe", scrollbarWidth: "none" }}
        >
          {tabs.map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className="whitespace-nowrap px-3 sm:px-5 py-2 sm:py-2.5 rounded-xl text-[11px] sm:text-[13px] font-semibold tracking-wide transition-all duration-200"
              style={
                activeTab === tab
                  ? { background: "linear-gradient(135deg, #1e3a8a 0%, #1E40AF 100%)", color: "white", boxShadow: "0 2px 10px rgba(30,64,175,0.35)" }
        ...
        <div className="flex flex-col gap-6 sm:gap-8">
          {sections.length > 0 ? (
            sections.map((section, i) => (
              <SectionBlock key={i} title={section.title} images={section.images} onImageClick={setLightboxSrc} />
            ))
          ) : (
            <div className="bg-white rounded-2xl py-16 text-center" style={{ border: "1.5px solid #dbeafe" }}>
